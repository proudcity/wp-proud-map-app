...
var link=document.createElement("a");link.href="#",link.className=active?"active":"",link.innerHTML=name,link.onclick=function(e){e.preventDefault(),e.stopPropagation();for(var key in mapLayers)map.removeLayer(mapLayers[key]);switch($("#menu-ui a.active").removeClass("active"),this.className="active",
...
map.invalidateSize(),type){case"wordpress":addWordpressLayer(query);
//attr.addAttribution('This website');
break;case"foursquare":if(-1!=query.indexOf(",")){query=query.split(",");for(var i=0;i<query.length;i++)addFoursquareLayer(query[i],"large")}else addFoursquareLayer(query,"large");
//attr.addAttribution('<a href="http://foursquare.com">Foursquare</a>');
map.attributionControl.setPrefix('Data from <a href="http://foursquare.com">Foursquare</a>');break;case"seeclickfix":addSeeClickFixLayer("closed"),addSeeClickFixLayer("open"),addSeeClickFixLayer("acknowledged"),map.attributionControl.setPrefix('Data from <a href="http://seeclickfix.com">SeeClickFix</a>')}},active&&$(link).trigger("click"),layers.appendChild(link)}function addWordpressLayer(query){var url=settings.wordpress.apiUrl+"?direction=ASC&filter%5Blocation-taxonomy%5D="+query+"&sort=date&per_page=100";$.getJSON(url,{},function(data){for(var geojson={type:"FeatureCollection",features:[]},i=0;i<data.length;i++){var item=data[i],properties=item.meta;if(properties.content=item.content?item.content.rendered:"",properties.featured_media=item.featured_media,void 0!=properties.lng&&void 0!=properties.lat){var marker=iconColor(properties.icon);$.extend(properties,marker,{title:item.title.rendered,"marker-size":"large"}),geojson.features.push({type:"Feature",properties:properties,geometry:{type:"Point",coordinates:[properties.lng,properties.lat]}})}}mapLayers[query]=L.mapbox.featureLayer().setGeoJSON(geojson),mapLayers[query].on("mouseover",function(e){e.layer.openPopup()}).on("mouseout",function(e){e.layer.closePopup()}).on("click",function(e){var props=e.layer.feature.properties;
// Grab the wp featured image
if(props.type="wordpress",props.location=props,props.location.postalCode=props.zip,props.googleLocal={hours:props.hours},$mapWrap.addClass("details-open"),$scope.details=props,$scope.$apply(),props.featured_media){var url=settings.wordpress.apiUrl+"../media/"+props.featured_media;$.getJSON(url,{},function(data){$scope.details.featured_media=data,$scope.$apply()})}}),mapLayers[query].addTo(map)})}function addFoursquareLayer(query,size){/* Query Foursquare API for venue recommendations near the current location. */
...
//'marker-size': size,
"marker-symbol":marker["marker-symbol"],url:void 0!=venues[i].venue.url?venues[i].venue.url:void 0,details:venues[i].venue},geometry:{type:"Point",coordinates:[venues[i].venue.location.lng,venues[i].venue.location.lat]}})}//for
...
$scope.details={type:"seeclickfix"},$scope.seeclickfix=props,$scope.$apply()}),mapLayers[query].addTo(map)})}function iconColor(category){var color="#997C61",icon="marker";return category=void 0!=category&&null!=category?category.toLowerCase():"",-1!=category.indexOf("librar")?(icon="library",color="#A973A9"):-1!=category.indexOf("school")||-1!=category.indexOf("college")||-1!=category.indexOf("university")?(icon="college",color="#ED9356"):-1!=category.indexOf("police")?(icon="police",color="#456D9C"):-1!=category.indexOf("fire")?(icon="fire-station",color="#E76C6D"):-1!=category.indexOf("post")?(icon="post",color="#5A97C4"):-1!=category.indexOf("parking")||-1!=category.indexOf("charging")?(icon="parking",color="#4DC3FF"):(-1!=category.indexOf("park")||-1!=category.indexOf("field")||-1!=category.indexOf("market"))&&(icon="park",color="#9BBF6A"),{"marker-symbol":icon,"marker-color":color}}if(_.has(settings,"lat")){var layers=document.getElementById("menu-ui"),mapLayers={};
// @todo: get from $rootSCope
L.mapbox.accessToken="pk.eyJ1IjoiYWxiYXRyb3NzZGlnaXRhbCIsImEiOiI1cVUxbUxVIn0.SqKOVeohLfY0vfShalVDUw";var map=new L.mapbox.Map("map","albatrossdigital.lpkdpcjb",{center:new L.LatLng(settings.lat,settings.lng),zoom:_.get(Proud,"settings.proud_map_app.instances."+$rootScope.appId+".zoom")||15,scrollWheelZoom:!1,zoomControl:!1});new L.Control.Zoom({position:"topright"}).addTo(map),window.map=map;var source=_.get(Proud,"settings.proud_map_app.instances."+$rootScope.appId+".source");if("wordpress"==source)var activeLayers=_.get(Proud,"settings.proud_map_app.instances."+$rootScope.appId+".wordpress_layers")||[];else{// defaults to Foursquare
var activeLayers=["foursquare:school,police station,fire station,library,post office,park:All","foursquare:school:Schools","foursquare:police station:Police","foursquare:fire station:Fire","foursquare:library:Libraries","foursquare:post office:Post Offices","foursquare:park:Parks"];"seeclickfix"==_.get(Proud,"settings.proud_map_app.global.311_service")&&activeLayers.push("seeclickfix:seeclickfix:Reported Issues")}activeLayers="string"==typeof activeLayers?activeLayers.split("\n"):activeLayers;var i=0;$.each(activeLayers,function(key,value){var layer=value.split(":");addLayer(layer[2],layer[0],null,layer[1],0==i),i++})}})}}})}(jQuery);